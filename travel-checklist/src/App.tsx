...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedItems) {
      try {
        const parsedItems: ChecklistItem[] = JSON.parse(savedItems)
        // Migrate existing items to include icons if they don't have them
        const itemsWithIcons = parsedItems.map(item => ({
          ...item,
